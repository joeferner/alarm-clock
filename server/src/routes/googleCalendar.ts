...
import {GoogleCalendarEvent, GoogleCalendarEventsResponse, GoogleCalendarSettings} from "./GoogleCalendarTypes";
...
export default function (app: express.Application) {
    ...
    app.get('/googleCalendar/events', function (req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        cache().get(CACHE_EVENTS_KEY, 10 * 60 * 1000, (callback) => {
            async.mapSeries(config.calendars, function (calendarId, callback) {
            ...
            }, (err: Error, results: GoogleCalendarEventsResponse[]) => {
                if (err) {
                    if ((<any>err).code == 401) {
                        return res.sendStatus(401);
                    }
                ...
                const allEvents = combineEvents(results);
                const v: GoogleCalendarEventsResponse = {
                    items: allEvents
                };
    ...
    function getEventsForCalendar(accessToken: string, calendarId: string, callback: (err: Error, data?: GoogleCalendarEventsResponse) => void): void {
        ...
        gcal(accessToken).events.list(calendarId, options, (err, data) => {
            ...
            const res: GoogleCalendarEventsResponse = data as GoogleCalendarEventsResponse;
